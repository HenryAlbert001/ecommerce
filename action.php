...
if (isset($_POST["count_item"])) {
	...
	// Khi người dùng đăng nhập thì chúng tôi sẽ đếm số lượng mặt hàng trong giỏ hàng bằng cách sử dụng id phiên của người dùng
	if (isset($_SESSION["uid"])) {
		$sql = "SELECT COUNT(*) AS count_item FROM cart WHERE user_id = $_SESSION[uid]";
	}else{
		//When user is not logged in then we will count number of item in cart by using users unique ip address
		// Khi người dùng chưa đăng nhập thì chúng tôi sẽ đếm số lượng mặt hàng trong giỏ hàng bằng cách sử dụng địa chỉ IP duy nhất của người dùng
...
// Lấy mặt hàng trong giỏ hàng từ cơ sở dữ liệu tới menu thả xuống
//Get Cart Item From Database to Dropdown menu
if (isset($_POST["Common"])) {

	if (isset($_SESSION["uid"])) {
		//When user is logged in this query will execute
		// Khi người dùng đăng nhập, truy vấn này sẽ thực thi
		$sql = "SELECT a.product_id,a.product_title,a.product_price,a.product_image,b.id,b.qty FROM products a,cart b WHERE a.product_id=b.p_id AND b.user_id='$_SESSION[uid]'";
	}else{
		//When user is not logged in this query will execute
		// Khi người dùng chưa đăng nhập, truy vấn này sẽ thực thi
		$sql = "SELECT a.product_id,a.product_title,a.product_price,a.product_image,b.id,b.qty FROM products a,cart b WHERE a.product_id=b.p_id AND b.ip_add='$ip_add' AND b.user_id < 0";
	}
	$query = mysqli_query($con,$sql);
	if (isset($_POST["getCartItem"])) {
		//display cart item in dropdown menu
		//hiển thị mục giỏ hàng trong menu thả xuống
		if (mysqli_num_rows($query) > 0) {
			$n=0;
			$total_price=0;
...
//cập nhật mặt hàng khỏi giỏ hàng
